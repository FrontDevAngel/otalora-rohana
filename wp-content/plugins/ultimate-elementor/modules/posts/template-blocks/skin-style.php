...
abstract class Skin_Style {
	...
	public function render_featured_excerpt() {
		...
		if ( '' == $_f_excerpt_length ) {

			$content = wp_trim_words( get_the_excerpt() );
		} else {

			$content = wp_trim_words( get_the_excerpt(), $_f_excerpt_length, ' ...' );
		}
	...
	public function render_term_html() {
		...
		if ( 'post' == $post_type ) {
			...
			$terms = wp_get_post_terms( get_the_ID(), $this->get_instance_value( 'terms_to_show' ) );
		} else {
	...
	public function get_body() {
		...
		$count         = 0;
		$is_featured   = false;
		$args          = self::$query_obj->query_posts_args();
		...
		$this->add_render_attribute( 'outer_wrapper', 'data-query', json_encode( $args ) );
